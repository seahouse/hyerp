...
@if (isset($paymentrequestretracts) && $paymentrequestretracts->count())
    @foreach($paymentrequestretracts as $item)
        <div class="reimbList list-group">
            <a href="{{ url($href_pre_paymentrequestretract . $item->id . $href_suffix) }}" class="list-group-item">
                ...
                <div class='col-xs-6 col-sm-6 content'
                     ...
                     <?php $sohead_taxamount = isset($sohead) ? $sohead->temTaxamountstatistics->sohead_taxamount : 0; ?>
                     <?php $sohead_poheadtaxamount = isset($sohead) ? $sohead->temTaxamountstatistics->sohead_poheadtaxamount : 0.0; ?>
                     @if (isset($item->purchaseorder_hxold->sohead) and $item->purchaseorder_hxold->sohead->receiptpayments->sum('amount') * 10000 < ($item->purchaseorder_hxold->sohead->payments->sum('amount') + $poheadAmountBy7550 + ($sohead_taxamount - $sohead_poheadtaxamount)))  style="color: #FF3300;") @endif >
                    <div title="{{ $item->applicant_name }}的付款" class="title">
                        <div class='longText'>{{ $item->paymenttype }} | {{ $item->amount }}</div>
                        {{-- 示例：山东奥博环保科技有限公司 --}}
                        {{-- @if (isset($item->supplier_hxold->name)) {{ $item->supplier_hxold->name }} @endif --}}
