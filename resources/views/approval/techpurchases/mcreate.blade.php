...
@section('title', '创建技术采购申请单')

@section('main')

@can('approval_techpurchase_create')
    {!! Form::open(array('url' => 'approval/techpurchase/mstore', 'class' => 'form-horizontal', 'id' => 'formMain', 'files' => true)) !!}
        @include('approval.techpurchases._form',
        	[
        		'submitButtonText' => '提交',
        		'project_name' => null,
        		'drawingchecker' => null,
            ...
            var item_num = 1;
			 ...
			 $("#btnSubmit").click(function() {
                ...
                $("div[name='container_item']").each(function(i){
                    ...
                    itemObject.item_type = container.find("input[name='item_type']").val();
...
//                        itemObject.weight = 0.0;
                    itemObject.descrip = container.find("input[name='descrip']").val();
                ...
                var itemHtml = '<div class="' + divName + '"><p class="bannerTitle">明细(' + String(item_num) + ')&nbsp;<button class="btn btn-sm" id="' + btnId + '" type="button">删除</button></p>\
                	<div name="container_item">\
						<div class="form-group">\
							<label for="item_name" class="col-xs-4 col-sm-2 control-label">商品名称:</label>\
						...
						<div class="form-group">\
							<label for="item_spec" class="col-xs-4 col-sm-2 control-label">商品型号:</label>\
							<div class="col-sm-10 col-xs-8">\
							<input class="form-control" readonly="readonly" name="item_spec" type="text" id="item_spec_' + String(item_num) + '">\
							</div>\
						</div>\
						<div class="form-group">\
							<label for="item_spec" class="col-xs-4 col-sm-2 control-label">商品类别:</label>\
							<div class="col-sm-10 col-xs-8">\
							<input class="form-control" readonly="readonly" name="item_type" type="text" id="item_type_' + String(item_num) + '">\
							</div>\
						</div>\
						<div class="form-group">\
						...
						<div class="form-group">\
							<label for="descrip" class="col-xs-4 col-sm-2 control-label">商品说明:</label>\
							<div class="col-sm-10 col-xs-8">\
							<input class="form-control" name="descrip" type="text"  id="descrip_' + String(item_num) + '">\
							</div>\
						</div>\
					</div>\
					</div>';
...
//                             alert(field.item_name);
                             strhtml += '<tr><td>' + field.item_name + '</td><td>' + field.item_spec + '</td><td>' + field.unit_name + '</td><td>' + field.size + '</td><td>' + field.quantity + '</td><td>' + field.weight + '</td><td>' + field.descrip + '</td><td>' + field.material + '</td></tr>'
